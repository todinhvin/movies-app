...
export async function getTivi(requestData) {
  const response = await fetch(
    `${DOMAIN_URL}/discover/tv?language=en-US&${requestData.params}&include_video=true&api_key=797d08dc86cbab298803421b8affd522&page=${requestData.pageNumber}&with_watch_monetization_types=flatrate`
  );
  const data = await response.json();
  if (!response.ok) {
    return new Error(data.message || "Load fail");
